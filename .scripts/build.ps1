...
# Check if target is running/locked
if (Test-Path $TARGET) {
    try {
        $file = [System.IO.File]::OpenWrite($TARGET)
        $file.Close()
    } catch {
        Write-Host "⚠️  WARNING: $TARGET is locked (may be running). Please close it before building." -ForegroundColor Yellow
        Write-Host "   The build will attempt to continue, but may fail if the file cannot be overwritten." -ForegroundColor Yellow
        Write-Host ""
    }
}
