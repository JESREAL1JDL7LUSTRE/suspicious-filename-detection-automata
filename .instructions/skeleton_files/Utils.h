 ...
 * 
 * Defines fundamental structures for NFA, DFA, PDA, and dataset entries
 */
...
/**
 * Represents a state in an automaton (NFA/DFA/PDA)
 */
struct State {
    int id;
    bool is_accepting;
...
/**
 * Represents a transition in NFA/DFA
 */
struct Transition {
    int from_state;
    int to_state;
    char symbol;
    bool is_epsilon;  // true for ε-transitions (NFA only)
...
struct DFA {
    std::vector<State> states;
    std::map<std::pair<int, char>, int> transition_table;  // (state, symbol) -> next_state
    int start_state;
    std::set<int> accepting_states;
    std::set<char> alphabet;
...
/**
 * Pushdown Automaton (for TCP validation)
 */
struct PDA {
    std::vector<State> states;
    ...
    PDA() : current_state(0), start_state(0) {
        pda_stack.push("BOTTOM");  // Stack bottom marker
    }
    
    void reset() {
        current_state = start_state;
        while (!pda_stack.empty()) pda_stack.pop();
        pda_stack.push("BOTTOM");
    }
    
    void push(const std::string& symbol) {
        pda_stack.push(symbol);
    }
    
    std::string pop() {
        if (pda_stack.size() > 1) {  // Don't pop BOTTOM
            std::string top = pda_stack.top();
            pda_stack.pop();
            return top;
        }
        return "";
    ...
    std::string peek() const {
        return pda_stack.empty() ? "" : pda_stack.top();
    }
    
    bool isAccepting() const {
        return accepting_states.count(current_state) > 0 && pda_stack.size() == 1;
    }
    
    int getStackDepth() const {
        return pda_stack.size() - 1;  // Exclude BOTTOM
    }
};
...
/**
 * Entry from malicious filename dataset
 */
struct FilenameEntry {
    ...
    bool is_malicious;  // For test cases
    
    FilenameEntry() : is_malicious(true) {}
};

/**
 * Entry from TCP handshake trace dataset
 */
struct TCPTrace {
    std::string trace_id;
...
/**
 * Performance metrics for DFA module
 */
struct DFAMetrics {
    int total_patterns;
    ...
    DFAMetrics() : total_patterns(0), total_nfa_states(0),
                   total_dfa_states_before_min(0), total_dfa_states_after_min(0),
                   total_dfa_states_after_iga(0), state_reduction_min_percent(0),
                   state_reduction_iga_percent(0), total_reduction_percent(0),
                   filenames_tested(0), true_positives(0), false_positives(0),
                   false_negatives(0), detection_accuracy(0),
                   avg_matching_time_ms(0), total_execution_time_ms(0),
                   estimated_memory_kb(0) {}
};

/**
 * Performance metrics for PDA module
 */
struct PDAMetrics {
    int total_traces;
    int valid_traces;
    ...
    PDAMetrics() : total_traces(0), valid_traces(0), invalid_traces(0),
                   correctly_accepted(0), correctly_rejected(0),
                   false_positives(0), false_negatives(0),
                   validation_accuracy(0), avg_stack_depth(0),
                   max_stack_depth(0), avg_validation_time_ms(0),
 ...
 */
inline void printSeparator(int length = 60) {
